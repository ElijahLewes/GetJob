...
def generate_cover_letter(job: Dict, resume_text: str, base_letter: str = "") -> str:
    """Generate a short cover letter for the job."""
    prompt = (
        f"Customize the following cover letter for the position {job['title']} at {job['company']}.\n"
        f"Job description: {job.get('description', '')}\n"
        f"Base cover letter:\n{base_letter}\n"
